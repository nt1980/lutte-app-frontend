...
const INPUT: React.CSSProperties = { width: '100%', background: 'var(--inp)', border: '1px solid var(--b3)', borderRadius: 10, padding: '8px 12px', fontSize: 13, color: 'var(--fg)', outline: 'none' };
...
export default function Clubs() {
  ...
  const deleteClub = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['clubs'] }); toast.success('Club supprimé'); },
    onError: () => toast.error('Erreur'),
  });

  const deleteAllClubs = useMutation({
    mutationFn: () => api.delete('/api/clubs'),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['clubs'] }); toast.success('Tous les clubs supprimés'); },
    onError: () => toast.error('Erreur'),
  });

  const handleEdit = (club: any) => {
    setEditingId(club.id);
    setForm({ fflda_number: club.fflda_number || '', short_name: club.short_name || '', name: club.name || '', city: club.city || '', regional_committee: club.regional_committee || '', coach_name: club.coach_name || '' });
    setModal(true);
  };

  const handleDeleteClub = (clubId: string) => {
    if (confirm('⚠️ Supprimer ce club ?')) deleteClub.mutate(clubId);
  };

  const handleDeleteAll = () => {
    if (confirm('⚠️ Êtes-vous sûr ? Cette action est irréversible.')) deleteAllClubs.mutate();
  };
    ...
    <Layout>
        ...
        <div style={{ background: 'var(--card)', border: '1px solid var(--b2)', borderRadius: 16, overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
                ...
                <tr key={c.id}>
                  <td style={TD}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div>
                        <div style={{ fontWeight: 600, color: 'var(--fg)' }}>{c.name}</div>
                        <div style={{ fontSize: 11, color: 'var(--faint)', marginTop: 1 }}>{c.short_name}</div>
                      </div>
                    </div>
                  </td>
                  <td style={TD}><span style={{ fontFamily: 'monospace', fontSize: 12, color: 'var(--fg3)', background: 'var(--inp)', padding: '2px 6px', borderRadius: 5 }}>{c.fflda_number || '—'}</span></td>
                  <td style={{ ...TD, color: 'var(--fg3)' }}>{c.city || '—'}</td>
                  <td style={{ ...TD, color: 'var(--faint)', fontSize: 12 }}>{c.regional_committee || '—'}</td>
                  <td style={{ ...TD, color: 'var(--fg3)' }}>{c.coach_name || '—'}</td>
              ...
              {clubs.length === 0 && (
                <tr><td colSpan={6} style={{ ...TD, textAlign: 'center', padding: '48px 16px', color: 'var(--fg3)' }}>Aucun club</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {modal && (
        <div style={{ position: 'fixed', inset: 0, background: 'var(--ovl)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: 16 }}>
          <div style={{ background: 'var(--card)', border: '1px solid var(--b3)', borderRadius: 20, width: '100%', maxWidth: 480, boxShadow: '0 40px 120px rgba(0,0,0,0.4)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px', borderBottom: '1px solid var(--b2)' }}>
              <div style={{ fontWeight: 700, color: 'var(--fg)', fontSize: 16 }}>{editingId ? 'Modifier le club' : 'Nouveau club'}</div>
              <button onClick={handleCloseModal} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--fg3)', display: 'flex' }}><X size={18} /></button>
            </div>
            <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', paddingTop: 4 }}>
                <button onClick={handleCloseModal} style={{ padding: '8px 16px', borderRadius: 9, background: 'var(--inp)', border: '1px solid var(--b3)', color: 'var(--fg2)', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>Annuler</button>
                <button onClick={() => create.mutate(form)} disabled={!form.short_name || !form.name || create.isPending}
