  ...
  return (
    <div style={{ minHeight: '100vh', background: '#080808', color: 'white', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '2rem 1.5rem' }}>
        ...
        {isLoading && (
          <div style={{ textAlign: 'center', padding: '4rem', color: '#6b7280' }}>Chargement du programme…</div>
        )}

        {isError && (
          <div style={{ textAlign: 'center', padding: '4rem' }}>
            <div style={{ color: '#f87171', fontWeight: 600, marginBottom: '0.5rem' }}>Programme non disponible</div>
            <div style={{ color: '#6b7280', fontSize: '0.875rem' }}>Le programme n'est pas encore publié pour ce tournoi.</div>
          </div>
        )}

        {!isLoading && !isError && competitions.length === 0 && (
          <div style={{ textAlign: 'center', padding: '4rem', color: '#6b7280' }}>
            <Users size={32} style={{ margin: '0 auto 1rem', display: 'block', color: '#374151' }} />
        ...
        {Object.entries(byCategory).map(([cat, comps]: [string, any]) => (
          <div key={cat} style={{ marginBottom: '3rem' }}>
            <div style={{ fontSize: '0.7rem', fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: '1rem' }}>
              {cat}
            </div>
            ...
            <div style={{ display: 'grid', gap: '1rem' }}>
              {comps.map((comp: any) => {
                ...
                return (
                  <div key={comp.id} style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, overflow: 'hidden' }}>
                    {/* Competition header */}
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.25rem', borderBottom: compPools.length ? '1px solid rgba(255,255,255,0.06)' : 'none' }}>
                      <div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ background: 'rgba(255,255,255,0.06)', borderRadius: 6, padding: '0.2rem 0.6rem', fontSize: '0.7rem', color: '#9ca3af' }}>
                    ...
                    {/* Pools */}
                    {compPools.length > 0 && (
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 1, background: 'rgba(255,255,255,0.04)' }}>
                        {compPools.map((pool: any) => (
                          <div key={pool.id} style={{ background: '#141414', padding: '1rem 1.25rem' }}>
                            <div style={{ fontWeight: 700, fontSize: '0.8rem', color: '#9ca3af', marginBottom: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                              {pool.name}
                            </div>
                              ...
                              {(pool.athletes || []).filter((a: any) => a.athlete_id).map((a: any, i: number) => (
                                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                  ...
                                  <div style={{ flex: 1, minWidth: 0 }}>
                                    <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'white', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{a.name}</div>
                                    <div style={{ fontSize: '0.7rem', color: '#6b7280' }}>{a.club}{a.weight ? ` · ${a.weight} kg` : ''}</div>
                                  </div>
                                </div>
                              ))}
                            </div>
                    ...
                    {compPools.length === 0 && (
                      <div style={{ padding: '1rem 1.25rem', color: '#4b5563', fontSize: '0.8rem', fontStyle: 'italic' }}>
