...
export default function MatManager() {
  ...
  // Pour un arbitre : seulement son tapis. Pour les autres : tous les tapis actifs non-jeunes.
  const activeMats = allMats.filter((m: any) => m.is_active !== false && !m.is_jeune);
  ...
  /* ── Données dérivées — on exclut les combats jeunes de ce gestionnaire ── */
  const standardQueue = queue.filter((q: any) => q.competition_source !== 'jeunes');

  const byMat = mats.reduce((acc: any, mat: any) => {
    const matches = standardQueue
