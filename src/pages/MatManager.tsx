...
export default function MatManager() {
  ...
  const pad = isMobile ? '12px 12px' : '20px 24px';
          ...
          <>
              ...
              {Object.values(byMat).map(({ mat, matches }: any) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      background: '#111',
                      border: `1px solid ${hasActivity ? 'rgba(251,191,36,0.25)' : 'rgba(255,255,255,0.06)'}`,
                      borderRadius: 14,
                      overflow: 'hidden',
                    }}
                  >
                    {/* Mat header */}
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: isMobile ? '14px 14px' : '14px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)', background: hasActivity ? 'rgba(251,191,36,0.05)' : 'transparent' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 36, height: 36, borderRadius: 10, background: hasActivity ? 'rgba(251,191,36,0.15)' : 'rgba(255,255,255,0.05)', border: `1px solid ${hasActivity ? 'rgba(251,191,36,0.3)' : 'rgba(255,255,255,0.08)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <Activity size={16} color={hasActivity ? '#fbbf24' : '#4b5563'} />
                        </div>
                        <div>
                          <div style={{ fontSize: 14, fontWeight: 700, color: '#fff' }}>{mat.name}</div>
                          {hasActivity
                            ? <div style={{ fontSize: 10, fontWeight: 700, color: '#fbbf24', marginTop: 1 }}>● EN COURS</div>
                            : <div style={{ fontSize: 10, color: '#374151', marginTop: 1 }}>Libre</div>
                          }
                        </div>
                      </div>
                      <Link
                        to={`/mat/${mat.id}`}
                        target="_blank"
                        style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12, fontWeight: 600, color: '#60a5fa', background: 'rgba(96,165,250,0.1)', border: '1px solid rgba(96,165,250,0.2)', borderRadius: 8, padding: '6px 12px', textDecoration: 'none' }}
                      >
                        <Tv size={12} /> Live
                      </Link>
                    </div>

                    <div style={{ padding: isMobile ? '12px 14px' : 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
                      {/* Current match */}
                      {current ? (
                        <div style={{ background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.15)', borderRadius: 10, padding: 12 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                            ...
                            <span style={{ flex: 1, fontSize: isMobile ? 14 : 13, fontWeight: 600, color: '#f87171' }}>{current.red_name || '?'}</span>
                            <span style={{ fontSize: 11, color: '#6b7280', fontWeight: 700 }}>vs</span>
                            <span style={{ flex: 1, fontSize: isMobile ? 14 : 13, fontWeight: 600, color: '#60a5fa', textAlign: 'right' }}>{current.blue_name || '?'}</span>
                          ...
                          {current.match_id && (
                            <Link
                              ...
                              target="_blank"
                              style={{ display: 'block', textAlign: 'center', fontSize: 12, fontWeight: 700, color: '#fff', background: '#dc2626', borderRadius: 8, padding: isMobile ? '10px 0' : '6px 0', textDecoration: 'none' }}
                            >
                              Vue arbitre →
                            </Link>
                          )}
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px 0', color: '#374151', fontSize: 12 }}>
                          Tapis libre
                        </div>
                      )}
                        ...
                        <div>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                            {next.slice(0, 3).map((q: any) => (
                              <div key={q.id} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 7, padding: isMobile ? '8px 10px' : '6px 10px', fontSize: 11 }}>
            ...
            {unassigned.length > 0 && (
              <div style={{ background: '#111', border: '1px solid rgba(248,113,113,0.15)', borderRadius: 14, overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: isMobile ? '12px 14px' : '14px 18px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                ...
                <div style={{ padding: isMobile ? '12px 14px' : 14, display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fill, minmax(260px, 1fr))', gap: 10 }}>
                  {unassigned.map((q: any) => (
                    <div key={q.id} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#ef4444', flexShrink: 0 }} />
                        <span style={{ flex: 1, fontSize: isMobile ? 13 : 12, fontWeight: 600, color: '#f87171', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{q.red_name || '?'}</span>
                        <span style={{ fontSize: 10, color: '#374151', fontWeight: 700 }}>vs</span>
                        <span style={{ flex: 1, fontSize: isMobile ? 13 : 12, fontWeight: 600, color: '#60a5fa', textAlign: 'right', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{q.blue_name || '?'}</span>
                      ...
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        {mats.map((mat: any) => (
                          <button
                            ...
                            style={{ fontSize: 12, fontWeight: 600, padding: isMobile ? '8px 14px' : '4px 10px', borderRadius: 7, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: '#d1d5db', cursor: 'pointer' }}
                          >
                            {mat.name}
                          </button>
                        ))}
