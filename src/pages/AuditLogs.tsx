...
const TH: React.CSSProperties = { padding: '10px 16px', textAlign: 'left', fontSize: 10, fontWeight: 700, color: '#4b5563', textTransform: 'uppercase', letterSpacing: '0.08em', borderBottom: '1px solid rgba(255,255,255,0.06)', whiteSpace: 'nowrap' };
const TD: React.CSSProperties = { padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.04)', verticalAlign: 'middle' };
        ...
        <div style={{ background: '#111', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, overflow: 'hidden' }}>
          {logs.length === 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '60px 24px', textAlign: 'center' }}>
              <Shield size={28} color="#374151" style={{ marginBottom: 10 }} />
              <div style={{ color: '#4b5563', fontSize: 13 }}>Aucune action enregistrée</div>
            </div>
          ) : (
                      ...
                      <tr key={l.id} style={{ background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)' }}>
                        ...
                        <td style={TD}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <div style={{ width: 26, height: 26, borderRadius: 7, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: '#9ca3af', flexShrink: 0 }}>
                              {l.user_name?.split(' ').map((w: string) => w[0]).join('').toUpperCase().slice(0, 2) || '?'}
                            </div>
                            <span style={{ fontSize: 13, color: '#d1d5db', fontWeight: 500 }}>{l.user_name || '—'}</span>
                          </div>
                        </td>
                        <td style={TD}>
                        ...
                        <td style={TD}>
                          <div style={{ fontSize: 12, color: '#9ca3af', fontWeight: 500 }}>{l.entity_type}</div>
                          {l.entity_id && (
                            <div style={{ fontSize: 10, color: '#374151', fontFamily: 'monospace', marginTop: 2 }}>{l.entity_id.slice(0, 8)}…</div>
                          )}
                        </td>
                      </tr>
                    );
