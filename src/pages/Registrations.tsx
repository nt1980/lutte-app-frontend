...
interface RegForm { athlete_id: string; final_age_category: string; final_style: string; }
    ...
    <Layout tournamentId={id}>
      <PageHeader
        ...
        actions={
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            {regs.length > 0 && (
              <button onClick={() => { if (confirm('⚠️ Êtes-vous sûr ?')) deleteAllMutation.mutate(); }} disabled={deleteAllMutation.isPending} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#ef4444', color: '#fff', padding: '8px 16px', borderRadius: 9, fontSize: 13, fontWeight: 600, border: 'none', cursor: deleteAllMutation.isPending ? 'not-allowed' : 'pointer', opacity: deleteAllMutation.isPending ? 0.5 : 1, boxShadow: '0 4px 12px rgba(239,68,68,0.3)' }}>
      ...
      <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ background: 'var(--card)', border: '1px solid var(--b2)', borderRadius: 16, overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>{['Combattant', 'Sexe', 'Licence', 'Club', 'Catégorie', 'Style', 'Pesée', 'Actions'].map(h => <th key={h} style={TH}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {isLoading && (
              ...
              {!isLoading && filtered.length === 0 && (
                <tr><td colSpan={8} style={{ ...TD, textAlign: 'center', padding: '64px 16px' }}>
                  ...
                  {!search && <div style={{ color: 'var(--faint)', fontSize: 12, marginTop: 4 }}>Importez un fichier CSV FFLDA pour commencer</div>}
                </td></tr>
              )}
              {filtered.map((r: any) => {
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <td style={TD}><span style={{ fontFamily: 'monospace', fontSize: 12, color: 'var(--fg3)', background: 'var(--inp)', padding: '2px 6px', borderRadius: 5 }}>{r.license_number || '—'}</span></td>
                    ...
                    <td style={{ ...TD, color: 'var(--fg3)', fontSize: 12, textTransform: 'capitalize' }}>{r.final_style || '—'}</td>
                    <td style={TD}>
                    ...
                    <td style={{ ...TD, textAlign: 'center' }}>
                      <button onClick={() => { if (confirm('⚠️ Supprimer cette inscription ?')) deleteRegMutation.mutate(r.id); }} style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', color: '#f87171', padding: '4px 8px', borderRadius: 5, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontSize: 12 }}>
      ...
      {showAddForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'var(--ovl)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: 16 }}>
          <div style={{ background: 'var(--card)', border: '1px solid var(--b3)', borderRadius: 20, width: '100%', maxWidth: 500, boxShadow: '0 40px 120px rgba(0,0,0,0.4)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px', borderBottom: '1px solid var(--b2)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  <div style={{ fontWeight: 700, color: 'var(--fg)', fontSize: 15 }}>Inscrire un combattant</div>
            ...
            <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--fg2)', marginBottom: 6 }}>Combattant *</label>
                <select value={formData.athlete_id} onChange={(e) => setFormData({...formData, athlete_id: e.target.value})} style={inputStyle}>
                  <option value="">Sélectionner…</option>
                  {athletes.map((a: any) => <option key={a.id} value={a.id}>{a.last_name} {a.first_name} ({a.license_number})</option>)}
                </select>
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--fg2)', marginBottom: 6 }}>Catégorie d'âge</label>
                <input type="text" value={formData.final_age_category} onChange={(e) => setFormData({...formData, final_age_category: e.target.value})} placeholder="ex: U13, U15…" style={inputStyle} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--fg2)', marginBottom: 6 }}>Style</label>
                <select value={formData.final_style} onChange={(e) => setFormData({...formData, final_style: e.target.value})} style={inputStyle}>
                  <option value="">—</option>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', padding: '16px 24px', borderTop: '1px solid var(--b2)' }}>
              ...
              <button onClick={() => addRegMutation.mutate(formData)} disabled={!formData.athlete_id || addRegMutation.isPending} style={{ padding: '8px 18px', borderRadius: 9, background: formData.athlete_id ? '#10b981' : '#065f46', color: '#fff', fontSize: 13, fontWeight: 600, border: 'none', cursor: formData.athlete_id ? 'pointer' : 'not-allowed' }}>
          ...
          <div style={{ background: 'var(--card)', border: '1px solid var(--b3)', borderRadius: 20, width: '100%', maxWidth: 640, boxShadow: '0 40px 120px rgba(0,0,0,0.4)' }}>
            ...
            <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, background: 'rgba(96,165,250,0.07)', border: '1px solid rgba(96,165,250,0.18)', borderRadius: 10, padding: '12px 14px', fontSize: 12, color: '#60a5fa' }}>
                ...
                <span>Collez le CSV exporté de FFLDA.</span>
              </div>
              <textarea
